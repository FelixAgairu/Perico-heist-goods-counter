...
const Counter = {
  ...
  getLoot: function() {
    ...
    Counter.secondaryTargetsOrder.forEach(element => {
      ...
      const clicks = (() => {
        ...
        return obj.name === 'paintings' ? `${value * 4} ${langCayoPerico[currLang].order_cuts}` : `${value} ${langCayoPerico[currLang].order_clicks}`;
      })();
  ...
  updateWebsite: function(amounts, totalValue, withinCooldownSecondaryBonus) {
    ...
    amounts.forEach(object => {
      ...
      if (amount !== 0) {
        element.innerHTML = `${amount} <span>${langCayoPerico[currLang]["sele_" + object.name] + langCayoPerico[currLang].order_bags} - ${object.clicks}</span>`;
